...
//public class DetectSurroundingJS : MonoBehaviour
//{
//    private GameObject[,,] blockObjs;
//    private GameObject[,,] prevBlockObjs;
//    private DetectSurroundingHS detectSurroundingHs;
...
//    {
//        Array.Clear(blockObjs,0,blockObjs.Length); 
...
//        blockObjs = detectSurroundingHs.GetTileMap();
...
//        for (int x = 0; x < blockObjs.GetLength(0); x++)
//        {
//            for (int y = 0; y < blockObjs.GetLength(1); y++)
//            {
//                for (int z = 0; z < blockObjs.GetLength(2); z++)
//                {
//                   //null 인경우는 pass
//                    if(blockObjs[x,y,z] == null) continue;
                    
//                    // 물체가 원래 아무것도 없는 곳으로 이동한 경우
//                    if (blockObjs[x, y, z] != null && prevBlockObjs[x, y, z] == null)
//                    {
//                        changedGameObj.Add(blockObjs[x, y, z]);
//                    }

//                    //물체가 원래 무언가 있는곳으로 이동한 경우
//                    if (blockObjs[x, y, z] != prevBlockObjs[x, y, z] && prevBlockObjs[x,y,z] != null)
//                    {
//                        changedGameObj.Add(blockObjs[x, y, z]);
//                    }
//                }
...
//            var adjacentObjects = detectSurroundingHs.GetAdjacentObjects(gameObject);
...
//    private void Awake()
//    {
//        detectSurroundingHs = DetectSurroundingHS.GetInstance;
//    }

//    private void Start()
//    {
        
//        StartCoroutine(WaitForInstantiateIndicator());
...
//    {
//        prevBlockObjs = new GameObject[blockObjs.GetLength(0), blockObjs.GetLength(1), blockObjs.GetLength(2)];
//        for (int i = 0; i < blockObjs.GetLength(0); i++)
//        {
//            for (int j = 0; j < blockObjs.GetLength(1); j++)
//            {
//                for (int k = 0; k < blockObjs.GetLength(2); k++)
//                {
//                    prevBlockObjs[i, j, k] = blockObjs[i, j, k];
//                }
//            }
//        }
//    }
...
//                    if(blockObjs[i,j,k] == null||blockObjs[i,j,k].tag != "InteractObj") continue;
//                    //특성값들 받아오기
//                    blockObjs[i, j, k].TryGetComponent(out InteractiveObject targetObj);
//                    //특성이 없으면 다음 오브젝트
//                    if(targetObj.Adjectives.Any() == false) continue;
//                    // 주체자인지 체크 하는 기능
//                    if(!IsInfluencer(targetObj)) continue;
//                    // 주체자면 검출 돌기
//                    var adjacentObjects = detectSurroundingHs.GetAdjacentObjects(blockObjs[i, j, k]);
//                    // 주변 게임오브젝트 도는 것
//                    foreach (var item in adjacentObjects)
//                    { 
                        
//                        if(item == null) continue; // 주변 게임 오브젝트가 널이면 뛰넘기
//                        if(item == blockObjs[i,j,k]) continue; // 주변 오브젝트가 자기자신이면 건너 뛰기
//                        //주변에 있는 오브젝트 특성 받아오기
//                        item.TryGetComponent(out InteractiveObject neighbor);
//                        //없으면 다음 주변에 있는에
//                        if(neighbor == null||neighbor.Adjectives.Any() == false) continue;
//                        var eachAbjs = CompareTwoAdjs(blockObjs[i, j, k], item);
//                        if (eachAbjs.Count!= 0)
//                        {
...
//                {
//                    // adjList.Add(influenceAdjs[i]);
//                    // adjList.Add(influencedAdjs[j]);
...
//    #region TestingGetObject with lockObj
//    public GameObject GetGameObjectWithIndex(int x, int y ,int z)
//    {
//        return blockObjs[x, y, z];
//    }

//    public List<int> GetIndexOrNullWithGameObject(GameObject targetObject)
//    {
//        for (int i = 0; i < blockObjs.GetLength(0); i++)
//        {
//            for (int j = 0; j <blockObjs.GetLength(1) ; j++)
...
//                for (int k = 0; k < blockObjs.GetLength(2); k++)
//                {
//                    if (blockObjs[i, j, k] == targetObject)
